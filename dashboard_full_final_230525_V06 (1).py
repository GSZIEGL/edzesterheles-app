...
# ⚽ Edzésterhelés Dashboard V06 – benchmark pizza, trend, új diagram, többfájlos betöltés
# Tartalmaz: benchmark pizzán és trendeken, Edzés vs Meccs vs Benchmark diagram, több .xlsx feltöltés
...
st.title("⚽ Edzésterhelés – Teljes Elemző Rendszer (V06)")
...
if not df_all.empty:
    ...
    st.header("📈 Trenddiagramok (benchmarkkal)")
    for player in selected_players:
        st.subheader(f"{player}")
        ...
        if not player_df.empty:
            ...
            for m in selected_metrics:
                if m in trend_df.columns:
                    ...
                    trend_df[f"Benchmark_{m}"] = normalize_series([benchmark_nominal.get(m)]*len(trend_df))
            fig = px.line(trend_df, x="Hét", y=[m for m in trend_df.columns if m not in ["Hét"]], markers=True, title=player)
            st.plotly_chart(fig, use_container_width=True)

    st.header("📊 Edzés vs Meccs vs Benchmark (összesítés)")
    new_chart = []
    for mut in selected_metrics:
        edzes_sum = df_filtered[df_filtered["Típus"] == "Edzés"][mut].sum()
        meccs_avg = df_filtered[df_filtered["Típus"] == "Meccs"][mut].mean()
        benchmark = benchmark_nominal.get(mut)
        new_chart.append({"Mutató": mut, "Típus": "Edzés összesen", "Érték": edzes_sum})
        new_chart.append({"Mutató": mut, "Típus": "Meccs átlag", "Érték": meccs_avg})
        new_chart.append({"Mutató": mut, "Típus": "Benchmark", "Érték": benchmark})
    df_newchart = pd.DataFrame(new_chart)
    fig = px.bar(df_newchart, x="Mutató", y="Érték", color="Típus", barmode="group", title="Edzés vs Meccs vs Benchmark")
    st.plotly_chart(fig, use_container_width=True)
